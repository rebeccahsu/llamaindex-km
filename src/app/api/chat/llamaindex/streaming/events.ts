...
export function appendSourceData(
  ...
  // console.log('--- appendSourceData sourceNodes---', JSON.stringify(sourceNodes));
...
function getNodeUrl(metadata: Metadata) {
  ...
  if (fileName && process.env.FILESERVER_URL_PREFIX) {
    ...
    if (filePath.startsWith(process.env.FILESERVER_URL_PREFIX)) {
      return filePath;
    }

    if (filePath.startsWith("https")) { // Google Drive URL
      return filePath;
    }
