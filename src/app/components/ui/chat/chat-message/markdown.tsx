...
const escapeNoSourcesTag = (content: string) => {
  return content.replace("[no-sources]", "");
};

const escapeTildes = (content: string) => {
    ...
    matches.forEach(match => {
      ...
      if (sourceNode !== undefined) {
        content = content.replace(
          match[0],
          `[citation:${matches.map((m) => m[1]).indexOf(sourceNode.id)}]()`,
        );
      } else {
...
const preprocessContent = (content: string, sources?: SourceData) => {
  return preprocessCitations(
    preprocessMedia(preprocessLaTeX(escapeNoSourcesTag(escapeTildes(content)))),
    sources,
  );
};
